...
# ===== Optional Gemini AI =====
USE_GEMINI = True
gemini_key = os.environ.get("GEMINI_API_KEY")  # المفتاح من Environment Variable
try:
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("All data is managed by admin via JSON files in `data/` folder.")

# ===== Tabs =====
tab_sim, tab_specs, tab_report, tab_ai = st.tabs(["🧮 Simulation", "📚 Specs Database", "📥 Export Report", "🤖 AI Guidance"])
...
with tab_sim:
    ...
    N_batches = math.ceil(total_panels / batch_size)
    processed_panels = []
    progress_bar = st.progress(0)
    for batch_idx in range(N_batches):
        start = batch_idx * batch_size
        end = min(start + batch_size, total_panels)
        batch = panels[start:end]
        for p_item in batch:
            p_item["processed_flag"] = True
            processed_panels.append(p_item)
        progress_bar.progress((batch_idx + 1)/N_batches)
        time.sleep(0.05)
    panels = processed_panels  # use processed
    ...
    if not (panel_opts and inv_opts and batt_opts):
        st.error("⚠️ Make sure data exists in JSON files")
    else:
...
# ===== Tab: Specs Database =====
with tab_specs:
    st.subheader("Company Specs (read-only)")
    for label, data_list in [("Panels", panels), ("Inverters", inverters), ("Batteries", batteries)]:
        st.markdown(f"#### {label}")
        if data_list:
...
with tab_report:
    ...
    if "last_result" not in st.session_state:
        ...
        st.download_button("⬇️ Download Excel", data=buffer.getvalue(), file_name="solar_report.xlsx")
        st.download_button("⬇️ Download CSV", data=df.to_csv(index=False).encode("utf-8"), file_name="solar_report.csv")

# ===== Tab: AI Guidance (Gemini) =====
with tab_ai:
    st.subheader("🤖 AI Guidance")
    if not USE_GEMINI:
        st.info("Gemini AI not configured or key missing.")
    elif "last_result" in st.session_state:
        res = st.session_state["last_result"]
        prompt = f"""
You are a senior solar design assistant. Validate and advise on sizing.
Inputs: {res['Inputs']}
Selected: {res['Selected']}
Computed: {res['Outputs']}
Provide guidance to reach 95–100% accuracy.
"""
        try:
            r = genai.GenerativeModel("gemini-1.5-flash").generate_content(prompt)
            st.write(r.text)
        except Exception as e:
            st.warning(f"Gemini guidance error: {e}")
    else:
        st.info("Run a simulation first to get AI guidance.")
